...
const stats = [
  { icon: Brain, value: "85%+", label: "Classification Accuracy" },
  { icon: Clock, value: "R² 0.82", label: "Processing Time Model" },
  { icon: Database, value: "25K+", label: "Records Analyzed" },
];
      ...
      <div className="container relative z-10 flex flex-col items-center text-center py-20">
        ...
        <div className="mb-8 inline-flex items-center gap-2 rounded-full border border-primary/30 bg-primary/10 px-4 py-1.5">
          ...
          <span className="font-heading text-xs tracking-wider text-primary">
            AI-Powered Prediction Engine v2.0
          </span>
        </div>

        {/* Title */}
        <h1 className="mb-4 text-5xl font-bold leading-tight text-foreground md:text-7xl">
          Visa Status{" "}
          <span className="text-gradient block">Prediction Engine</span>
        </h1>

        <p className="mb-3 text-lg text-muted-foreground max-w-xl">
          AI-Enabled Visa Status & Processing Time Estimator
        </p>
        <p className="mb-10 text-sm text-muted-foreground/70 max-w-2xl">
          Leverage Gradient Boosting & Random Forest models trained on 25K+
          historical records to forecast outcomes and estimate timelines.
        </p>
        ...
        <div className="grid w-full max-w-3xl grid-cols-1 gap-4 sm:grid-cols-3">
          {stats.map((stat) => (
            <div
              key={stat.label}
              className="glass-card flex flex-col items-center gap-2 p-6 animate-pulse-glow"
            >
